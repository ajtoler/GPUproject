...
int main() {
	...
	h_A = (int *)malloc(area);
	h_B = (int *)malloc(area);
	h_A_s = (int *)malloc(area);
	h_B_s = (int *)malloc(area);
	h_C = (int *)malloc(area);
	...
	cudaMemcpy(d_A, h_A, area, cudaMemcpyHostToDevice);
	cudaMemcpy(d_B, h_B, area, cudaMemcpyHostToDevice);
	cudaMemcpy(d_A_s, h_A_s, area, cudaMemcpyHostToDevice);
	cudaMemcpy(d_B_s, h_B_s, area, cudaMemcpyHostToDevice);
	cudaMemcpy(d_C, h_C, area, cudaMemcpyHostToDevice);
	...
	stencil_2d<<<grid,block>>>(d_A + RADIUS * length + RADIUS, d_A_s + RADIUS * length + RADIUS, length);
	stencil_2d<<<grid,block>>>(d_B + RADIUS * length + RADIUS, d_B_s + RADIUS * length + RADIUS, length);

	// Copy results back to host
	cudaMemcpy(h_A_s, d_A_s, area, cudaMemcpyDeviceToHost);
	cudaMemcpy(h_B_s, d_B_s, area, cudaMemcpyDeviceToHost);
	...
	dot_product<<<grid2, block2>>>(d_A_s, d_B_s, d_C, length);

	// Copy results back to host
	cudaMemcpy(h_C, d_C, area, cudaMemcpyDeviceToHost);
